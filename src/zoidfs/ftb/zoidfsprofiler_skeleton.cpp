...
{
    int ret = 0;
    Pzoidfs_getattr(handle, attr, op_hint);
    return ret;
}
...
{
    int ret = 0;
    ret = Pzoidfs_create(parent_handle, component_name, full_path, sattr,
                   ...
                   zoidfs_op_hint_t * op_hint)
{
    ...
    ret = Pzoidfs_symlink(from_parent_handle, from_component_name,
          from_full_path, to_parent_handle, to_component_name, to_full_path,
          sattr, from_parent_hint, to_parent_hint, op_hint);
    return ret;
}
...
{
    int ret = 0;
    ret = Pzoidfs_mkdir(parent_handle, component_name, full_path, sattr,
          parent_hint, op_hint);
    return ret;
}
...
{
    int ret = 0;
    ret = Pzoidfs_readdir(parent_handle, cookie, entry_count_, entries, flags,
          parent_hint, op_hint);
    return ret;
}
