...
static void hash_table_free_entry(HashTable *hash_table, HashTableEntry *entry)
{
	...
	if (hash_table->key_free_func != NULL) {
		hash_table->key_free_func(hash_table->user_ptr, entry->key);
	}

	/* Likewise with the value */

	if (hash_table->value_free_func != NULL) {
		hash_table->value_free_func(hash_table->user_ptr,
                      entry->value);
	}
...
static int hash_table_enlarge(HashTable *hash_table)
{
	...
	for (i=0; i<old_table_size; ++i) {
		...
		while (rover != NULL) {
			...
			index = hash_table->hash_func(hash_table->user_ptr, rover->key) % 
...
int hash_table_insert(HashTable *hash_table, HashTableKey key, HashTableValue value) 
{
	...
	index = hash_table->hash_func(hash_table->user_ptr, key) % hash_table->table_size;
	...
	while (rover != NULL) {
		if (hash_table->equal_func(hash_table->user_ptr, rover->key, key) != 0) {
			...
			if (hash_table->key_free_func != NULL) {
				hash_table->key_free_func(hash_table->user_ptr, 
                                      rover->key);
			}
...
HashTableValue hash_table_lookup(HashTable *hash_table, HashTableKey key)
{
	...
	while (rover != NULL) {
		if (hash_table->equal_func(hash_table->user_ptr, key, rover->key) != 0) {
