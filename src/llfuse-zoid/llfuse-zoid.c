...
#define ZFUSE_INITIAL_INODES 128

#define ZFUSE_XATTR_ZOIDFSHANDLE "user.zoidfs_handle"

/* the following is C99 safe */ 
#ifndef NDEBUG
#define zfuse_debug(format, ...) fprintf (stderr, "zfuse: " format,\
                                        ##__VA_ARGS__)
#else
#define zfuse_debug(format, ...) {}
#endif

#define zfuse_error(format, ...) fprintf (stderr, "zfuse: " format,\
                                        ##__VA_ARGS__)
...
typedef signed long  zfuse_inode_num_t; 

#define ZFUSE_INODE_INVALID ((zfuse_inode_num_t) -1)
/* The inode tracking is used because fuse only allows us to store
...
typedef struct 
{
   ...
   union
   {
      // Used when the inode is in use
      zoidfs_handle_t handle;

      // Used when the inode is not in use
      zfuse_inode_num_t nextfree; 
   }
   unsigned long   generation; 
} zfuse_inode_t; 


static zfuse_inode_t *     zfuse_inodes  = NULL; 
static zfuse_inode_num_t   zfuse_inode_capacity = 0; 
static zfuse_inode_num_t   zfuse_inode_free = 0; 
static zfuse_inode_num_t   zfuse_inode_used = 0; 

static int zfuse_inode_step (zfuse_inode_num_t inode, zfuse_inode_num_t count)
{
   assert (inode >= 0 && inode <= zfuse_inode_capacity); 

   /* fancy check to ensure refcounts do not go negative */ 
   assert (zfuse_inodes[inode].refcount >=  -count); 
   zfuse_inodes[inode].refcount += count; 
   return 1; 
}

/*
 * Adds inode, sets refcount to 1
 */
static zfuse_inode_num_t zfuse_inode_add (const zoidfs_handle_t * handle)
{
  inode_t newpos; 
   int i; 
   ...
   if (!zfuse_handles)
   {
      zfuse_debug ("Initializing zfuse_inodes\n"); 
      zfuse_inode_num_t i; 

      zfuse_inodes = calloc (ZFUSE_INITIAL_INODES, sizeof (zfuse_inode_t));
      zfuse_inode_used = 0; 
      zfuse_inode_capacity = ZFUSE_INITIAL_INODES; 
      zfuse_inode_free = 0; 

      assert (zfuse_inode_capacity); 

      /* init free list */
      for (i=0; i<zfuse_inode_capacity; ++i)
      {
         zfuse_inodes[i].nextfree = 
            (i == zfuse_inode_capacity - 1 ? -1 : i+1); 
      }
   }

   /* check for full */ 
   if (zfuse_inode_used == zfuse_inode_capacity)
   {
      /* extend */ 
      int oldcap = zfuse_inode_capacity; 

      /* if it is full, we cannot have a free list */ 
      assert (zfuse_inode_free == ZFUSE_INODE_INVALID); 

      zfuse_inode_capacity *= 2; 
      zfuse_inodes =
          realloc (zfuse_inodes, sizeof(zfuse_inode_t)*zfuse_inode_capacity);

      for (i=oldcap; i<zfuse_inode_capacity; ++i)
      {
         fuse_inodes[i].nextfree = 
            (i == zfuse_inode_capacity - 1 ? -1 : i+1); 
      }

      zfuse_inode_free = oldcap; 
   }

   assert (zfuse_inode_used < zfuse_inode_capacity); 
   assert (zfuse_inode_free != ZFUSE_INODE_INVALID && 
         zfuse_inode_free < zfuse_inode_capacity); 

   newpos = zfuse_inode_free; 

   zfuse_inode_free = zfuse_inodes[zfuse_inode_free].nextfree; 
   ++zfuse_inode_used; 
   zfuse_inodes[newpos].handle = *handle; 

   zfuse_debug ("inode %i added as %s (in use %i)\n", newpos, 
         zoidfs_handle_string (handle), zfuse_inode_used); 
   return newpos; 
}

/*
 * Decreases refcount of inode by step.
 * If it gets to zero, the inode is removed 
 */
static void zfuse_inode_remove (zfuse_inode_num_t pos, zfuse_inode_num_t step)
{
   assert (pos >= 0 && pos < zfuse_inode_capacity); 
   assert (zfuse_inode_used); 

   zfuse_debug ("Decreasing refcount on inode %i (before decrease = %li)\n", 
         pos, zfuse_inodes[pos].refcount); 

   assert (zfuse_inodes[pos].refcount); 

   zfuse_debug ("removing inode %i [%s] (in use %i)\n", pos, 
         zoidfs_handle_string(&zfuse_inodes[pos].handle),
         zfuse_inode_used); 


   assert (step < zfuse_inodes[pos].refcount); 

   zfuse_inodes[pos].refcount -= step; 

   if (zfuse_inodes[pos].refcount)
      return; 

   /* inode gets deleted: add to free list and increase generation */
   ++zfuse_inodes[pos].generation; 
   zfuse_inodes[pos].nextfree = zfuse_inode_free; 
   zfuse_inode_free = pos; 
   --zfuse_inode_used; 
}

static const zfuse_inode_t * zfuse_inode_retrieve (zfuse_node_num_t pos)
{
   assert (pos >= 0 && pos < zfuse_inode_capacity); 
   assert (zfuse_inodes[pos].refcount); 
   return &zfuse_inodes[pos]; 
}



/* ======================================================================= */
/* ========= POSIX / ZOIDFS util functions =============================== */
/* ======================================================================= */
...
void zfuse_ll_forget (fuse_req_t req, fuse_ino_t ino, unsigned int nlookup)
{
   zfuse_inode_remove (ino, nlookup); 
}

void zfuse_ll_lookup (fuse_req_t req, fuse_ino_t parent, const char * name)
{
   const zfuse_inode_t * parent = zfuse_inode_retrieve (parent); 
   zoidfs_handle_t handle; 
   int ret = zoidfs_lookup (parent->handle, name, NULL, &handle); 
...
static struct fuse_lowlevel_ops zfuse_ll_ops = 
{
   .init                = zfuse_ll_init,
   .destroy             = zfuse_ll_destroy,
...
int main(int argc, char *argv[])
{
    ...
    if (fuse_parse_cmdline(&args, &mountpoint, NULL, NULL) != -1 &&
        ...
        se = fuse_lowlevel_new(&args, &zfuse_ll_ops, sizeof(zfuse_ll_ops),
                               NULL);
        if (se != NULL) {
