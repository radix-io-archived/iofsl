...
namespace iofwdclient
{
   ...
   int SyncClient::getattr (const zoidfs::zoidfs_handle_t * handle,
         zoidfs::zoidfs_attr_t * attr, zoidfs::zoidfs_op_hint_t * op_hint)
   {
      zoidfs::zoidfs_request_t req;
      int ret = asclient_.igetattr (&req, handle, attr, op_hint);
      return (ret == zoidfs::ZFS_OK ? waitOp (req, zoidfs::ZFS_COMP_DONE) : ret);
   }
    ...
    * Releases r
    */
   int SyncClient::waitOp (zoidfs::zoidfs_request_t r, zoidfs::zoidfs_comp_mask_t mask)
   {
      // Automatically release r, no matter what
      ScopedRequest req (r);

      int ret = asclient_.request_test (req, ZOIDFS_TIMEOUT_INF, mask);
      ASSERT (ret == zoidfs::ZFS_OK);
