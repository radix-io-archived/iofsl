...
error_report(){
  ...
  echo | mutt -c committer_email rjdamore@gmail.com dkimpe@mcs.anl.gov drieskimpe@gmail.com -a make_error_report.txt -s "make error: Make FAIL for commit $comm" io-fwd-discuss@lists.mcs.anl.gov
  else test_report 
  fi
...
test_report(){
  ...
  awk 'NR<=5' runtest_results.txt >> test_report.txt ; egrep 'Running|PASS|All' runtest_results.txt >> test_report.txt ; awk '/--Run/{c=4}c&&c--' runtest_results.txt >> test_report.txt; tail -5 runtest_results.txt >> test_report.txt
  echo "==========================================================" >> test_report.txt
  ...
  echo | mutt -c rjdamore@gmail.com drieskimpe@gmail.com dkimpe@mcs.anl.gov -a test_report.txt -s "test report for commit $comm PASS" io-fwd-discuss@lists.mcs.anl.gov 
  edit_files  
}  
