...
void aarpcGetNextOffsetClientHelper(RPCClientHandle h,
...
void aarpcCreateOffsetClientHelper(RPCClientHandle h,
        zoidfs::zoidfs_handle_t & handle,
        zoidfs::zoidfs_file_ofs_t offset,
        uint64_t & retcode)
{
...
void aarpcDeleteOffsetClientHelper(RPCClientHandle h,
        zoidfs::zoidfs_handle_t & handle,
        uint64_t & retcode)
{
   SingleCompletion block;
...
int main (int argc, char ** args)
{
   try
   {
      ...
      if (!opt_remote.empty())
      {
         ...
         uint64_t retcode = 0;
         zoidfs::zoidfs_handle_t handle;
         zoidfs::zoidfs_file_size_t incsize = 1000000;
         zoidfs::zoidfs_file_size_t offset = 0;

         /* create the offset */
         aarpcCreateOffsetClientHelper(rpcclient->rpcConnect
                 ("aarpc.createoffset", addr), handle, offset, retcode);

         for (size_t i=0; i<10; ++i)
         {
             /* get the next offset, inc based on the cur size */
             aarpcGetNextOffsetClientHelper(rpcclient->rpcConnect
                     ("aarpc.getnextoffset", addr), handle, incsize, offset,
                     retcode);
         ...
         /* remove the offset */
         aarpcDeleteOffsetClientHelper(rpcclient->rpcConnect
                 ("aarpc.deleteoffset", addr), handle, retcode);
