...
int main(int argc, char **argv)
{
    ...
    int i, numproc, rank, ret, numfiles;
    zoidfs_op_hint_t op_hint;
    char rankfilename[ZOIDFS_PATH_MAX];
    ...
    if(argc < 3)
    {
        ...
        fprintf(stderr, "%s:%i should be similar to 'zoidfs-aa-write \
            /dir/to/store/file/in numfiles'\n", __func__, __LINE__);
    ...
    /* rank will create the file. if the file exists, if will delete it and recreate an empty file */
    if(rank < numfiles)
    {
        int created = 0;

        /* create the file */
        sprintf(rankfilename, "%s.%i", filename, rank);
        ret = zoidfs_create(&basedir_handle, rankfilename, NULL, &sattr,
        ...
        {
            /* remove the file */
            ret = zoidfs_remove(&basedir_handle, rankfilename, NULL, NULL, ZOIDFS_NO_OP_HINT);
            if(ret != ZFS_OK)
            {
    ...
    else
    {
        ...
        sprintf(rankfilename, "%s.%i", filename, rank % numfiles);
        ret = zoidfs_lookup(&basedir_handle, rankfilename, NULL, &handle, ZOIDFS_NO_OP_HINT);
        if(ret != ZFS_OK)
        {
    ...
    zoidfs_file_ofs_t fofs = BUFS_PER_RANK * (rank % numfiles) * buffer_size; 
    ...
    for( i = 0 ; i < BUFS_PER_RANK ; i++)
    {
        ...
        fprintf(stderr, "%s:%i file id = %i rank = %i, orig offset = %lu, append \
            offset = %lu\n", __func__, __LINE__, rank % numfiles, rank, fofs, newoff);
        fofs += buffer_size; 
