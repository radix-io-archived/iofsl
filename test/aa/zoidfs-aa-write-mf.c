...
int iofsl_setup(int my_rank, int my_size, int * num_servers)
{
    int i = 0;
    int my_server = 0;
    char ** server_list = NULL;
    char * server_string = NULL;
    char my_hostname[HOSTNAMELEN];

    int cps = 0;

    /* get the server info */
    *num_servers = atoi(getenv("NUM_ZOIDFS_SERVERS"));
    server_list = (char **)malloc(sizeof(char *) * (*num_servers));
    server_string = getenv("ZOIDFS_SERVER_LIST");

    cps = my_size / *num_servers;

    /* get the server list */
    for(i = 0 ; i < *num_servers ; i++)
    {
        server_list[i] = strsep(&server_string, "%");
    }

    /* get hostname info and server mapping */
    gethostname(my_hostname, HOSTNAMELEN);
    my_server = (int)floor(1.0 * (*num_servers) * my_rank / my_size);

    /* set the hostname */
    fprintf(stderr, "rank = %i, ZOIDFS_ION_NAME = %s\n", my_rank, 
            server_list[my_server]);
    setenv("ZOIDFS_ION_NAME", server_list[my_server], 1);
...
int main(int argc, char **argv)
{
    ...
    int num_file_groups = 2;
    int num_server_groups = 0;
    int file_group = 0;
    int server_group = 0;
    ...
    /* assign this rank to a file group */
    file_group = (int)floor(1.0 * num_file_groups * rank / numproc);

    /* setup the IOFSL runtime env variables */
    server_group = iofsl_setup(rank, numproc, &num_server_groups);

    /* check args */
    if(argc < 2)
    {
        fprintf(stderr, "%s:%i incorrect args\n", __func__, __LINE__);
        fprintf(stderr, "%s:%i should be similar to 'zoidfs-aa-write-mf /dir/to/store/file/in'\n", __func__, __LINE__);
    ...
    sprintf(filename, "zoidfs-write-aa-mf.%i.txt", file_group);
    fprintf(stderr, "rank = %i, filename = %s\n", rank, filename);
    //if(rank % (numproc / num_file_groups) == 0)
    if(1)
    {
        int created = 0;
    ...
    zoidfs_file_ofs_t fofs = BUFS_PER_RANK * (rank % num_file_groups) * buffer_size; 
    ...
    for( i = 0 ; i < BUFS_PER_RANK ; i++)
    {
        ...
        /* output file write info... where did the client specify the
           ...
           file (new offset) */
        fprintf(stderr, "%s:%i rank = %i, orig offset = %lu, append offset = %lu fn = %s\n",
                __func__, __LINE__, rank, fofs, newoff, filename);
        fofs += buffer_size; 
