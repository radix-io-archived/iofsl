/*
 * zoidfsapi-link-test.c
 * Test the ZOIDFS link test.
 */
...
int main(int argc, char **argv) {
    ...
    if(argc < 2) {
        fprintf(stderr, "Usage: zoidfsapi_link_test <mount point>\n");
        exit(1);
    }
    ...
    snprintf(link_local, NAMESIZE, "link-comp");
    snprintf(link_fullpath_target, NAMESIZE, "%s/test-zoidfs-fullpath", argv[1]);
    snprintf(link_component_target, NAMESIZE, "test-zoidfs-component");
    ...
    /* create links  using the component path*/
    ret = zoidfs_link(&basedir_handle, link_local, NULL, &basedir_handle, link_component_target, NULL,
                         NULL, NULL);
    if(ret != ZFS_OK) {
        goto exit;
    }
    ...
    /* create links  using the full path*/
    ret = zoidfs_link(NULL, NULL, link, NULL, NULL, link_fullpath_target,
                         NULL, NULL);
    if(ret != ZFS_OK) {
        goto exit;
    }

    ret = zoidfs_lookup(NULL, NULL, link, &shandle);
    if(ret != ZFS_OK) {
            goto exit;
    }
    ...
    {
        fprintf(stderr, "fullpath test: link handle and original file handle are the same\n");
    }
    else
    {
        fprintf(stderr, "fullpath test: link handle and original file handle differ\n");
    }

    /* Now check the attrs of the link and the file handle */
    ...
    {
        fprintf(stderr, "fullpath test: file attributes are the same for the link and the original file\n");
    }
    else
    {
        fprintf(stderr, "fullpath test: file attributes differ for the link and the original file\n");
    }
    ...
    ret = zoidfs_lookup(&basedir_handle, link_local, NULL, &shandle);
    if(ret != ZFS_OK) {
            goto exit;
    }
    ...
    if(memcmp(&shandle, &fhandle, sizeof(zoidfs_handle_t)) == 0)
    {
        fprintf(stderr, "component handle: link handle and original file handle are the same\n");
    }
    else
    {
        fprintf(stderr, "component handle: link handle and original file handle differ\n");
    }
    ...
    if(compare_attr(&resattr_file, &resattr_link))
    {
        fprintf(stderr, "component test: file attributes are the same for the link and the original file\n");
    }
    else
    {
        fprintf(stderr, "component test: file attributes differ for the link and the original file\n");
    }

    /* Remove the files and links */
    ret = zoidfs_remove(&basedir_handle, component_filename, NULL, NULL);
    if(ret != ZFS_OK) {
        goto exit;
    }
    ...
    ret = zoidfs_remove(&basedir_handle, link_local, NULL, NULL);
    if(ret != ZFS_OK) {
        goto exit;
    }
