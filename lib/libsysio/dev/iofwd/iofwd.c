...
static struct inode_ops iofwd_i_ops = {
    ...
    inop_old_sync: iofwd_inop_sync,
    inop_old_datasync: iofwd_inop_datasync,
...
static ssize_t iofwd_filldirentries(struct pnode *pno, _SYSIO_OFF_T *posp, char *buf, size_t nbytes)
{
    ...
    SYSIO_IOFWD_FINFO("ec = %lu", entry_count);
    if(err != ZFS_OK)
    {
        SYSIO_IOFWD_FEXIT();
        return -zfs_error_to_sysio_error(err);
    ...
    *posp = entries[entry_count - 1].cookie;
    ...
    for(i = 0 ; i < entry_count ; i++)
    {
        ...
        SYSIO_IOFWD_FINFO("off = %lu reclen = %i name = %s", outdp[i]->d_off, outdp[i]->d_reclen, outdp[i]->d_name);
